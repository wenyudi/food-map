...
export default function Stepper({ value, onChange, min = 1, max = 30, unit = '人', full = false }: Props) {
  ...
  return (
    ...
    >
      <button onClick={dec} className="w-7 h-7 rounded-full border-2 border-on-surface bg-white text-lg font-bold leading-none press-sm">
        −
      </button>
      ...
      <button onClick={inc} className="w-7 h-7 rounded-full border-2 border-on-surface bg-accent text-lg font-bold leading-none press-sm">
        +
      </button>
    </div>
  )
